...
export function Question2({ onSave, savedData, savedObservations }: Question2Props) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      onSave({ birthday }, observations);
    }, 500);
    return () => clearTimeout(timer);
  }, [birthday, observations]);

  const handleVirtualInput = (text: string) => {
    if (activeInput === 'birthday') {
      setBirthday(prev => prev + text);
  ...
  const handleVirtualBackspace = () => {
    if (activeInput === 'birthday') {
      setBirthday(prev => prev.slice(0, -1));
      if (birthdayInputRef.current) {
        birthdayInputRef.current.focus();
      }
    } else {
    ...
    <div className="bg-white rounded-xl shadow-lg p-8 space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          2 - ESCREVA A DATA DO SEU ANIVERSÁRIO (DIA E MÊS)
        </h2>
        ...
        <textarea
          ...
          onFocus={() => setActiveInput('birthday')}
          className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition min-h-[120px] text-lg ${
            activeInput === 'birthday' ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
          }`}
