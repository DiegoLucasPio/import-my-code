...
export function Question4A({ onSave, onNext, savedData }: Question4AProps) {
  ...
  const handleKeyboardInput = (text: string) => {
    if (activeInput === 'sentence1') {
      setSentence1(prev => prev + text);
    }
  };

  const handleKeyboardBackspace = () => {
    if (activeInput === 'sentence1') {
      setSentence1(prev => prev.slice(0, -1));
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">
        4A - COMPLETE AS FRASES DO SEU JEITO.
      </h2>
        ...
        <div className="flex items-center gap-4 bg-pink-50 p-6 rounded-lg">
          <div className="flex-shrink-0">
            <div className="w-24 h-24 flex items-center justify-center">
              <img src={imgPrincess} alt="Princesa" className="w-full h-full object-contain" />
            </div>
          </div>
          <div className="flex-1">
            <div className="text-xl font-medium flex items-center gap-2">
              ...
              <input
                ...
                onFocus={() => setActiveInput('sentence1')}
